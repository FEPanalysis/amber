...
def workingOnFileObject(function):
   ...
   def convert_if_needed(f, *args, **kwargs):
      with (open(f, 'r') if isinstance(f, str) else f) as infile:
...
def readDataAmber(P):
   ...
   # Names of the DV/DL components in the MDOUT file.
   ...
   def parseFile(fn):
	 ...
	 # dvdl components, and number of the dvdl entries.
         fn_ene,          = grepFromSection(f, 'File Assignments', 'MDEN')[1]
         fn_ene = "%s/%s" % (fn.rpartition('/')[0], fn_ene)
         dt,              = grepFromSection(f, 'Molecular', 'dt')[1]
         logdvdl, clambda = grepFromSection(f, 'Free energy', 'logdvdl', 'clambda')[1]
         DVDL             = grepFromSection(f, 'DV/DL,', *dvdl_components)[1]
